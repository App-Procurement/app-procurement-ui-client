...
class ViewInvoice extends Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      this.props.get_invoice_status === status.SUCCESS
    ) {
      this.setState({
        approvedData: this.props.get_invoice_data,
      });
    }
  }
  displayTableData = () => {
    ...
    let retData = [];
    if (approvedData.invoiceItem && approvedData.invoiceItem.length > 0) {
