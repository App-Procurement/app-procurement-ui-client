...
class SetUpCommittee extends Component {
  ...
  componentDidMount() {
    if (
      this.props.selected_member_list &&
      this.props.selected_member_list.committeeMember &&
      this.props.selected_member_list.committeeMember.length > 0
    ) {
      this.setState({
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      (prevProps.selected_committee_status !==
        this.props.selected_committee_status &&
        this.props.selected_committee_status === status.SUCCESS) ||
      (prevProps.selected_member_list !== this.props.selected_member_list &&
        this.props.selected_committee_status === status.SUCCESS)
    ) {
      this.setState({
        committeeMember: this.props.selected_member_list.committeeMember,
    ...
    if (
      prevProps.get_committee_type_status !==
        this.props.get_committee_type_status &&
      this.props.get_committee_type_status === status.SUCCESS
    ) {
    ...
    if (
      prevProps.add_committee_status !== this.props.add_committee_status &&
      this.props.add_committee_status === status.SUCCESS
    ) {
      let sendData = {
        committeeMember: [],
        userid: 5,
                ...
                >
                  ...
                  <div className="settings-toggle">
                    {displayOption && i === activeindex ? (
                      <span
                        onClick={(id) => this.onClickCommitteeItemDelete(i)}
                      >
            ...
            {committeeMember && committeeMember.length > 0 && (
              <div className="pt-4 select-members">
                ...
                <div className="membar-list">
                  <div className="row">{this.displayCommiteeLists()}</div>
                </div>
                <div className="d-block text-center committee-btn">
                  <Button
                    variant="contained"
                    className="primary-btn"
                    onClick={this.addCommitteeMember}
                  >
...
const connectedSetUpCommittee = withTranslation()(
  connect(mapStateToProps)(SetUpCommittee)
);
