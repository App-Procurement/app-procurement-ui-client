...
class TrackRfp extends Component {
  ...
  componentDidMount() {
    this.props.dispatch(
      recievedrfpAction.getTrackRfpData({ id: this.props.match.params.id })
    );
  }

  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.track_frp_status !== this.props.track_frp_status &&
      this.props.track_frp_status === status.SUCCESS
    ) {
      if (
        this.props.track_frp_data &&
        this.props.track_frp_data.trackUpdates.length > 0
      ) {
        this.setState({
  ...
  handleTableData = (recievedData) => {
    ...
    if (
      recievedData &&
      recievedData.trackUpdates &&
      recievedData.trackUpdates.length > 0
    ) {
          ...
          <TableRow key={i}>
            ...
            {data && (
              <TableCell align="right">
                {data.department}
                <br />
                {new Date(data.orderedDate).toLocaleDateString()}
                <br />
                {new Date(data.orderedDate).toLocaleTimeString("en-US")}
              </TableCell>
            )}
          </TableRow>
        );
  ...
  render() {
    ...
    const { orderedDate, poGenerateDate, approvedDate, deliveryDate } =
      recievedData;
    return (
            ...
            <div className="track-progress-bar">
              ...
              <ul className="progress-contant">
                <li>
                  ...
                  {orderedDate && (
                    <p>{new Date(orderedDate).toLocaleTimeString("en-US")}</p>
                  )}
                </li>
                <li>
                  ...
                  {approvedDate && (
                    <p>{new Date(approvedDate).toLocaleTimeString("en-US")}</p>
                  )}
                </li>
                <li>
                  ...
                  {poGenerateDate && (
                    <p>
                      {" "}
                      {commonFunctions.convertDateToString(
                        new Date(poGenerateDate)
                      )}
                    </p>
                  )}
                  {poGenerateDate && (
                    <p>
                      {new Date(poGenerateDate).toLocaleTimeString("en-US")}
                    </p>
                  )}
                </li>
                <li>
                  ...
                  {deliveryDate && (
                    <p>{new Date(deliveryDate).toLocaleTimeString("en-US")}</p>
                  )}
                </li>
              </ul>
            </div>
