...
class CreateRooms extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (
      ...
      if (this.props.get_members_data && this.props.get_members_data.length > 0) {
        for (let i = 0; i < this.props.get_members_data.length; i++) {
          this.props.get_members_data[i]['checked'] = this.props.get_members_data[i].checked
    ...
    if (
      this.props.create_chat_room_status !== prevProps.create_chat_room_status &&
  ...
  validateForm = (isSubmitted) => {
    ...
    if (isSubmitted) {
      const { membersList, roomName, description } = this.state;
  ...
  createRoom = () => {
    ...
    if (validateForm.isSubmited) {
      ...
      this.props.dispatch(ChatRoomActions.createChatRoom({ members, description, roomName }));
    }
            ...
            <div className="row">
              <div className="col-xl-4 col-lg-4 col-md-5 col-sm-12">
                <div className="create-chatroom-cards ">
                  ...
                  <SimpleBar className="create-chatroom-inner-section">{this.handleRooms()}</SimpleBar>
                </div>
              </div>
                ...
                <div className="create-chatroom-right-section">
                  ...
                  <div className="create-chatroom-inner-content">
                    <div className="room-name">
                      ...
                      />
                      <span className="d-block text-danger">{validateForm.roomName.message}</span>
                    </div>
                    <div className="room-name">
                      ...
                      <textarea
                        ...
                        onChange={this.handleChange}
                      />
                      <span className="d-block text-danger">{validateForm.description.message}</span>
                    </div>
                    <div className="add-members">
                      ...
                      <div className="member-list">
                        ...
                        <SimpleBar className="member-list-inner">
                          {membersList && membersList.length > 0 ? (
                            membersList.map(({ name, id, level, checked }, index) => (
                              <div className="member-content" key={`${index}_member_content`}>
                                ...
                                <FormControlLabel
                                  ...
                                  control={
                                    <Checkbox
                                      ...
                                      value={checked}
                                      checked={checked}
                                      onChange={(e) => this.selectMembers(index, e)}
                                    />
                                  }
                                />
                              </div>
                      ...
                      <span className="d-block w-100 text-danger">{validateForm.membersList.message}</span>
                    </div>
                    <div className="create-chatroom-button">
                      <label></label>
                      <div className="create-chatroom-button-section text-center">
                        <Button variant="contained" className="cancel-btn" onClick={this.clearFields}>
                          Cancel
                        </Button>
                        <Button variant="contained" className="create-btn" onClick={this.createRoom}>
                          Create
                        </Button>
                      </div>
                    </div>
