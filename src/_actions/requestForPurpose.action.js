    ...
    requestForPurposeServices.addRequest(data).then(
      (response) => {
        if (response.code === 200) {
          ...
          alert.success("Added Reuqest Successfully");
        } else {
          dispatch(
            dispatchFunction({
              type: status.FAILURE,
  ...
  return (dispatch) => {
    dispatch(
      dispatchFunction({
        type: status.IN_PROGRESS,
        data: {
          products_list_status: status.IN_PROGRESS,
          products_list: null,
        },
      })
    );
    requestForPurposeServices.getProductsList().then(
      (response) => {
        if (response.code === 200) {
          dispatch(
            dispatchFunction({
              type: status.SUCCESS,
              data: {
                products_list_status: status.SUCCESS,
                products_list: response.object,
              },
            })
          );
        } else {
          dispatch(
            dispatchFunction({
              type: status.FAILURE,
              data: {
                products_list_status: status.FAILURE,
  ...
  return (dispatch) => {
    dispatch(
      dispatchFunction({
        type: status.IN_PROGRESS,
        data: {
          add_products_list_status: status.IN_PROGRESS,
          add_products_list_response: null,
        },
      })
    );
    requestForPurposeServices.addProductsList(data).then(
      (response) => {
        if (response.code === 200) {
          dispatch(
            dispatchFunction({
              type: status.SUCCESS,
              data: {
                add_products_list_status: status.SUCCESS,
                add_products_list_response: response.object,
              },
            })
          );
        } else {
          dispatch(
            dispatchFunction({
              type: status.FAILURE,
              data: {
                add_products_list_status: status.FAILURE,
                add_products_list_response: response,
              },
            })
          );
          alert.error(response.message);
      ...
      (error) => {
        dispatch(
          dispatchFunction({
            ...
            data: {
              add_products_list_status: status.FAILURE,
              add_products_list_response: error.message,
            },
          })
        );
        alert.error(error.message);
